...
public class StatisticsController {
    ...
    public ResponseEntity<?> getSalesStatistics(@RequestParam(defaultValue = "day") String period) {
        ...
        // 转换为前端需要的格式
        List<Map<String, Object>> result = salesData.entrySet().stream()
                .map(entry -> {
    ...
    public ResponseEntity<?> getTopBooks(@RequestParam(defaultValue = "10") int limit) {
        ...
        // 构建结果
        List<Map<String, Object>> result = bookSales.entrySet().stream()
    ...
    public ResponseEntity<?> getCategorySales() {
        ...
        // 构建结果
        List<Map<String, Object>> result = categoryQuantities.entrySet().stream()
                .map(entry -> {
                    Map<String, Object> item = new HashMap<>();
    ...
    public ResponseEntity<?> getOrderStatusDistribution() {
        ...
        if (orders.isEmpty()) {
            ...
            return ResponseEntity.ok(Collections.singletonList(defaultItem));
        }
